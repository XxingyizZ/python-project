...
# Context（上下文）是模型本次请求能够看到的相关信息集合。
# 它可以包括用户问题、instructions、历史消息、Memory或工具结果。
# 后续的RAG检索结果也会作为Context的一部分提供给模型。

# 文本会被模型转换成token，模型一次请求能够处理的Context有范围限制。
# 因此历史消息不能无限增长，RAG通常只取Top-K，工具结果也要保持简洁。
# 本阶段只建立基本认识，不学习tokenizer的底层实现。
